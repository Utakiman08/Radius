...
fullscreen(){
  ...
  ShowModal(SiteId: string): void {
    ...
    sessionStorage.setItem( `TORRENTsiteId`,SiteId)
...
private autoZoomMode(specialMarkers: L.Marker[]): void {
    ...
    <div class="donut-cluster">
      <svg viewBox="0 0 42 42" class="donut">
        <circle class="donut-segment" cx="21" cy="21" r="15.91549431" fill="transparent"
          ...
          stroke-dasharray="${livePercentage} ${ 100 - livePercentage}" stroke-dashoffset="0"></circle>
        <circle class="donut-segment" cx="21" cy="21" r="15.91549431" fill="transparent"
          stroke="#dc3545" stroke-width="10"
          stroke-dasharray="${maintenancePercentage} ${100 - maintenancePercentage}" stroke-dashoffset="-${livePercentage}"></circle>
      </svg>
      <div class="donut-count" [ngClass]="darkMode ? 'dark-mode' : 'light-mode'" >${count}</div>
    </div>`;
    return L.divIcon({
      html: donutHtml,
  ...
  locations = [
    {'lat': 27.2078536024158, 'lng': 77.9639142680996, 'name': '', 'city': '', 'site': '', 'siteId': '625a850f73ea07.27134758',  'sensorcount':0 , 'diccount': 0, 'datalogger_count': 0, 'Grid': "", 'DG': "",'DG_RUNNING': [{'name': '', 'last_reading_updated_duration': ''}]},
    {'lat': 27.2284929959411, 'lng': 77.9972341601528, 'name': '', 'city': '', 'site': '', 'siteId': '624ec162ac9740.39132103', 'sensorcount': 0, 'diccount': 0, 'datalogger_count': 0, 'Grid': "", 'DG': "",'DG_RUNNING': [{'name': '', 'last_reading_updated_duration': ''}]},
  ];
}
